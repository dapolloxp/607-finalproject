...
For the weather data processing, I used a CSV file from the weather.gov site. However, due to the fact that there several years included as part of this data, I decided to store it in MongoDB and retrieve is via a simple MongoDB query

The password is stored locally on a file for security reasons and the MongoDB instance is opened to only a few select IPs.
...
lines(xx, predict(fit6, data.frame(MONTHLY_TEMP=xx)), col="purple")
```
