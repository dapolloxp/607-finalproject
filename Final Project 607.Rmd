...
For the *weather data*, we downloaded a .CSV and then placed into a MongoDB instance running in Microsoft Azure. We then filtered by year with the query: weather.2017<-mgo$find('{"YEAR":{"$gt":2007}}'). The values were then stored into an R dataframe and converted from wide to long format using tidyverse gather.
...
Combining the data required that the year and month in both dataframes had matching variables and titles. We then did a left join on the dataframes and renames the columns to keep the column names in a standard format. 
...
# this polynomial has the best fit, with R2 of approx. 13%
fit6<-lm(DAYS_ON_TIME ~ poly(MONTHLY_TEMP,6, raw=TRUE), agg.combined)
summary(fit6) 
...
lines(xx, predict(fit10, data.frame(MONTHLY_TEMP=xx)), col="purple")
...
Predicting NYC subway delays is fairly difficult. There are several factors that could potentially contribute to massive train delays, for example:
...
In this example, we choose to take one factor, weather, and determine what, if any impact it could have on train delays. Based on our analysis, we could only account for 12% of the variability of % Days on-time. In order to come up with a better model, we would need more data and additional analysis.
