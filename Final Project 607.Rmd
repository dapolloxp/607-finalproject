...
new$PERIOD_MONTH<- as.integer(new$PERIOD_MONTH)
...
# aggregrate temperature values monthly
agg.temp<-aggregate(list(MONTHLY_TEMP=combined$MONTHLY_TEMP), by = list(MONTH=combined$PERIOD_MONTH), FUN=mean)

# aggregate OTP and ridership
...
# scatter plot 
scatter.smooth(x=agg$MONTHLY_TEMP, y=agg$MONTH_VALUE)

# boxplot to see outliers
par(mfrow=c(1, 2))  # divide graph area in 2 columns
boxplot(aggr$`Overall monthly OTP`,sub=paste("Outlier rows: ", boxplot.stats(aggr$`Overall monthly OTP`)$out))  
boxplot(aggr$`Overall monthly temp`, sub=paste("Outlier rows: ", boxplot.stats(aggr$`Overall monthly temp`)$out))  

# density plot 
library(e1071)
par(mfrow=c(1, 2))  # divide graph area in 2 columns

plot(density(aggr$`Overall monthly OTP`), main="Density Plot: Monthly OTP", ylab="Frequency", sub=paste("Skewness:", round(e1071::skewness(aggr$`Overall monthly OTP`), 3)))  
polygon(density(aggr$`Overall monthly OTP`), col="red")

plot(density(aggr$`Overall monthly temp`), main="Density Plot: Monthly Temp", ylab="Frequency", sub=paste("Skewness:", round(e1071::skewness(aggr$`Overall monthly temp`), 3)))  
polygon(density(aggr$`Overall monthly temp`), col="red")

# the correltion is very weak between OTP and temp, approximately 21% - is there another variable from the initial dataset that will work better?
cor(aggr$`Overall monthly OTP`, aggr$`Overall monthly temp`) 
...
# all factors considered gives us 87.96
fit <- lm( `Monthly Actual`~ `Avg Monthly Temp` + `Monthly Target` + Subway + Year , data=combined)
summary(fit) 
plot(fit)

fit2<- lm( `Monthly Actual`~ `Avg Monthly Temp` + Subway, data=combined)
summary(fit2) 
plot(fit2)

fit3<- lm( `Monthly Actual`~ `Monthly Target`, data=combined)
summary(fit3) 
plot(fit3)

fit4<- lm( `Monthly Actual`~ Subway, data=combined)
summary(fit4) 
plot(fit4)

fit5<- lm( `Monthly Actual`~ Year, data=combined)
summary(fit5) 
plot(fit5)
